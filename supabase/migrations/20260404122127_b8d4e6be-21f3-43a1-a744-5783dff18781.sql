...
BEGIN
  ...
  -- ── Country-aware normalization to mg/dL (standard) ──
  -- Creatinine: Uzbekistan stores µmol/L, India stores mg/dL
  _cr := COALESCE(_lab.creatinine, 0);
  IF _country = 'uzbekistan' AND _cr > 10 THEN
    _cr := ROUND(_cr / 88.4, 2);
  END IF;

  -- Bilirubin: Uzbekistan stores µmol/L, India stores mg/dL
  ...
  ELSE -- kidney
    ...
    IF _best_creatinine IS NOT NULL AND _best_creatinine > 0
       AND _cr > 0
       AND _cr > _best_creatinine * 1.25 THEN
      _score := _score + 35;
    END IF;

    -- eGFR (mL/min/1.73m² — universal)
  ...
  RETURN jsonb_build_object(
    ...
    'algorithm_version', 'v3.1-country-aware'
  );
END;
$function$;